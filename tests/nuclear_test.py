...
from drudge.nuclear import JOf, MOf, CG, Wigner6j, Wigner3j, _simpl_pono_term
...
def test_varsh_872_4(nuclear: NuclearBogoliubovDrudge):
    ...
    for sums_i in [sums, reversed(sums)]:
        ...
        assert term.amp == KroneckerDelta(c, c_prm) * KroneckerDelta(
            gamma, gamma_prm
        )
...
def test_varsh_872_5(nuclear: NuclearBogoliubovDrudge):
    ...
    sums = [(alpha, Range("m", -a, a + 1)), (gamma, Range("M", -c, c + 1))]
    amp = CG(a, alpha, b, beta, c, gamma) * CG(
        a, alpha, b_prm, beta_prm, c, gamma
    )

    expected = (
        KroneckerDelta(b, b_prm)
        * KroneckerDelta(beta, beta_prm)
        * (2 * c + 1)
        / (2 * b + 1)
    )
    for sums_i in [sums, reversed(sums)]:
...
def test_varsh_911_8(nuclear: NuclearBogoliubovDrudge):
    ...
    m_range = Range("m")
    sums = [
        (m_i, m_range[-j_i, j_i + 1])
        for m_i, j_i in [(m1, j1), (m2, j2), (m3, j3), (m12, j12), (m23, j23)]
    ]
    amp = (
    ...
    expected = (
        KroneckerDelta(j, j_prm)
        * KroneckerDelta(m, m_prm)
        * (-1) ** (j1 + j2 + j3 + j)
        * sqrt(2 * j12 + 1)
        * sqrt(2 * j23 + 1)
...
def test_wigner3j_sum_to_wigner6j(nuclear: NuclearBogoliubovDrudge):
    ...
    sums = [
        (m_i, m_range[-j_i, j_i + 1])
        for m_i, j_i in [(m1, j1), (m2, j2), (m4, j4), (m5, j5), (m6, j6)]
    ]

    phase = (-1) ** (j1 + j2 + j4 + j5 + j6 - m1 - m2 - m4 - m5 - m6)
    amp = (
